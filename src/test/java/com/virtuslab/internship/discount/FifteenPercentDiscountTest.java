...
public class FifteenPercentDiscountTest {
    ...
    void shouldApply15PercentDiscountWhen3GrainProducts(){
        ...
        var expectedTotalPrice = bread.price().multiply(BigDecimal.valueOf(2))
                .add(cereal.price()).multiply(BigDecimal.valueOf(0.85));
        // When
    ...
    @Test
    void shouldNotApply15PercentDiscountWhenNot3GrainProducts(){
        var productDb = new ProductDb();
        var bread = productDb.getProduct("Bread");
        var apple = productDb.getProduct("Apple");
        List<ReceiptEntry> receiptEntries = new ArrayList<>();
        receiptEntries.add(new ReceiptEntry(bread, 2));
        receiptEntries.add(new ReceiptEntry(apple, 3));

        var receipt = new Receipt(receiptEntries);
        var discount = new FifteenPercentDiscount();
        var expectedTotalPrice = bread.price().multiply(BigDecimal.valueOf(2))
                .add(apple.price().multiply(BigDecimal.valueOf(3)));
